...
def extract_heading_lines():
    ...
    def build_patterns():
        return [
            ...
            ("ARTICLE",  r"^第\s*([一二三四五六七八九十百0-9]+)\s*條(?:\s*之\s*\d+)?"),
    ...
    def classify_line(line, patterns, prev_line=None):
        is_block_start = (
            ...
            prev_line.strip() == "" or
            prev_line.strip().startswith("[page") or
        ...
        for tag_type, pattern in patterns:
            ...
            if m:
                ...
                if not is_block_start and tag_type not in inline_ok_types:
                    if len(remainder) > 20:
                        continue
                    if "所稱" in remainder:
                        continue
    ...
    def is_duplicate_heading(prev_info, curr_info):
        if not prev_info or not curr_info:
            return False
        if prev_info["type"] != curr_info["type"]:
            return False
        if prev_info.get("number") != curr_info.get("number"):
            return False
        return True
    ...
    def extract_blocks(lines, patterns, fixed, dynamic):
        ...
        while idx < len(lines):
            ...
            info = classify_line(stripped, patterns, prev_line=prev_line)

            if info and current_block:
                previous_heading = current_block[0].strip()
                prev_info = classify_line(previous_heading, patterns)
                if is_duplicate_heading(prev_info, info):
                    current_block[0] = line
                    if new_lines:
                        new_lines[-1] = line
                    if structure_lines:
                        structure_lines[-1] = f"[{info['token']}]"
                    idx += 1
                    continue
