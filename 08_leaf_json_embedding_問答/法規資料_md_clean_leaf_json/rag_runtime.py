...
def load_embedding_data(mode: str, hybrid_text_mode: str = "leaf") -> tuple[np.ndarray, list[dict[str, Any]], dict[str, Any]]:
    if mode in COMBINED_MODES:
        source_modes = COMBINED_MODES[mode]
        ...
        for source_mode in source_modes:
            src_embeddings, src_metadata, src_summary = load_single_embedding_data(source_mode)
            source_record_counts[source_mode] = len(src_metadata)
            source_summaries[source_mode] = {
                "metadata": src_summary.get("files", {}).get("metadata")
                if isinstance(src_summary.get("files"), dict)
                else None,
                "record_count": len(src_metadata),
            }
            embeddings_list.append(src_embeddings)
            for row in src_metadata:
                row_with_source = dict(row)
                row_with_source["_source_mode"] = source_mode
                merged_metadata.append(row_with_source)

        merged_embeddings = np.concatenate(embeddings_list, axis=0)
        merged_summary = {
            "mode": mode,
            "combined_sources": list(source_modes),
